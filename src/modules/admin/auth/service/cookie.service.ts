...
export class CookieService {
  ...
  setAuthCookies(res: Response, { accessToken, refreshToken }) {
    ...
    res.cookie(CookieService.ACCESS_TOKEN_NAME, accessToken, { ...commonOptions, maxAge: 60 * 60 * 1000 });
    res.cookie(CookieService.REFRESH_TOKEN_NAME, refreshToken, { ...commonOptions, maxAge: 60 * 60 * 1000 });
  }
  ...
  cleanAuthCookies(res: Response) {
    ...
    res.clearCookie(CookieService.ACCESS_TOKEN_NAME, options);
    res.clearCookie(CookieService.REFRESH_TOKEN_NAME, options);
  }
}
