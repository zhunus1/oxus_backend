...
export class CreateEventDto {
  @ApiProperty({ example: "NIS Shymkent Presentation" })
  @IsString({ message: messages.MUST_BE_STRING("title") })
  @IsNotEmpty({ message: messages.REQUIRED_FIELD("title") })
  title: string;

  @ApiProperty({ example: "nis-shymkent-2026" })
  @IsString({ message: messages.MUST_BE_STRING("slug") })
  @IsNotEmpty({ message: messages.REQUIRED_FIELD("slug") })
  slug: string;

  @ApiProperty({ example: "2026-03-02T22:00:00Z" })
  @IsDateString({}, { message: messages.MUST_BE_DATE("eventDate") })
  @IsNotEmpty({ message: messages.REQUIRED_FIELD("eventDate") })
  eventDate: string;
