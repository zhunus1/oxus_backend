# Server image shared unchanged by the Test and Production environments.
# Environment-specific configuration and secrets are supplied only at runtime.
...
COPY . .
RUN DATABASE_URL=postgresql://postgres:postgres@localhost:5432/postgres yarn prisma generate && yarn build
